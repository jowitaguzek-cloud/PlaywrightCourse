...
test('Test API - GET', async ({ page, request }) => {
...
test('Test API - PUT', async ({ request }) => {
  ...
  expect(response.ok()).toBeTruthy();
  console.log(await response.text())
  expect(await response.text()).toContain('{\"message\":\"replaced (mock)\",\"product\":{\"id\":3,\"name\":\"Zmieniony\",\"price\":111.11},\"note\":\"No persistence. This is a mock response.\"}')
...
test('Test API - DELETE', async ({ page, request }) => {
